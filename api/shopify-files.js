module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'replace_file') {
      ...
      // Step 4: Update file with staged resource
      console.log('Updating file in Shopify...');
      
      const updateMutation = 'mutation { fileUpdate(files: [{ id: "' + fileId + '", originalSource: "' + stagedTarget.resourceUrl + '" }]) { files { id } userErrors { field message } } }';
      ...
      if (updateData.data.fileUpdate.userErrors.length > 0) {
        console.error('File update errors: ' + JSON.stringify(updateData.data.fileUpdate.userErrors));
        return res.status(500).json({ 
