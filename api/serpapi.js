  ...
  try {
    ...
    for (const searchQuery of searchPatterns) {
      ...
      if (paaQuestions.length > 0) {
        ...
        // Filter to only questions that contain the niche keyword
        const filteredQuestions = paaQuestions.filter(q => {
          const questionLower = q.question.toLowerCase();
          const nicheLower = niche.toLowerCase();
          return questionLower.includes(nicheLower);
        });
        
        console.log(`${filteredQuestions.length} questions contain "${niche}"`);
