...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('[Money Page] Step 1: Finding competitors...');
    ...
    console.log('[Money Page] Step 2: Analyzing your page...');
    const yourPageData = await analyzePage(pageUrl, keyword);

    // Step 3: Analyze competitor pages
    console.log('[Money Page] Step 3: Analyzing competitors...');
    const competitorData = [];
    for (let i = 0; i < competitors.length && i < 4; i++) {
      console.log(`[Money Page] Analyzing competitor ${i + 1}...`);
    ...
    console.log('[Money Page] Step 4: Getting AI recommendations...');
...
async function analyzePage(url, keyword) {
  try {
    ...
    // Get PageSpeed scores (with rate limiting)
    await sleep(1000);
    const speedMobile = await getPageSpeedScore(url, 'mobile');
    await sleep(1000);
    const speedDesktop = await getPageSpeedScore(url, 'desktop');
...
Analyze the gaps and provide prioritized recommendations. Return ONLY a JSON object in this exact format:
