...
function injectReoptimiseContext(prompt, note) {
  const block = `\n\n⚠️ REOPTIMISE MODE — this page was ALREADY optimised once and has since LOST ranking for its target keyword. ${note}\nYour FIRST job is to INVESTIGATE why it dropped. Work out the SINGLE most likely reason: over-optimisation (the exact keyword stuffed too hard in the title, headings or body), a competitor overtaking it (compare the competitor content provided), the search intent shifting, thin or duplicated content, or lost internal links. Then make every recommendation CORRECT that specific cause. Do NOT just repeat generic "use the keyword more" advice — the keyword may already be over-used, so prefer natural variations, a stronger match to what searchers want, and better content depth.\nALSO add a top-level JSON field "dropDiagnosis" — a plain-English 1–2 sentence verdict for a non-technical shop owner naming the SINGLE most likely reason this page dropped AND the one main fix (e.g. "A competitor published a longer, fresher guide and overtook you — add sections on X and Y and refresh the article date."). Keep the exact JSON format described below and just add that one extra field.\n`;
  if (/^You are [^\n]*\n/.test(prompt)) return prompt.replace(/^(You are [^\n]*\n)/, `$1${block}`);
  return block + prompt;
}
