...
module.exports = async function handler(req, res) {
  ...
  try {
    const { pageUrl, keyword, ubersuggestImage, gscData } = req.body;
    ...
    if (gscData) {
      console.log('[Money Page] GSC data provided - will check for cannibalization');
    }

    // Step 1: Check for cannibalization using GSC data
    let cannibalizationData = null;
    if (gscData && gscData.opportunities) {
      console.log('[Money Page] Step 1: Checking for keyword cannibalization...');
      cannibalizationData = await detectCannibalization(pageUrl, keyword, gscData.opportunities);
      if (cannibalizationData.hasConflict) {
        console.log(`[Money Page] ⚠️ CANNIBALIZATION DETECTED: ${cannibalizationData.competingPages.length} pages competing for "${keyword}"`);
      } else {
        console.log(`[Money Page] ✓ No cannibalization detected for "${keyword}"`);
      }
    }

    // Step 2: Find competitors using SerpAPI and check user position
    console.log('[Money Page] Step 2: Finding competitors... (~10 sec)');
    ...
    console.log('[Money Page] Step 3: Analyzing your page... (~15 sec)');
    ...
    console.log('[Money Page] Step 4: Analyzing competitors... (~15 sec)');
    ...
    console.log('[Money Page] Step 5: Analyzing content gaps...');
    ...
    // Step 6: Get Claude analysis
    console.log('[Money Page] Step 6: Getting AI recommendations... (~20 sec)');
    const analysis = await getClaudeAnalysis(yourPageData, competitorData, keyword, searchResults.userPosition, ubersuggestImage, contentGaps, cannibalizationData);
...
// Detect keyword cannibalization using GSC data
async function detectCannibalization(pageUrl, keyword, gscOpportunities) {
  try {
    ...
    // Find all pages ranking for this keyword (or very similar keywords)
    const competingPages = gscOpportunities.filter(opp => {
      const oppKeywordLower = opp.keyword.toLowerCase();
      const oppUrlNormalized = normalizeUrl(opp.url);
      
      // Exact keyword match or very close match (90% similarity)
      const isKeywordMatch = oppKeywordLower === keywordLower || 
                             oppKeywordLower.includes(keywordLower) || 
                             keywordLower.includes(oppKeywordLower);
      
      return isKeywordMatch && oppUrlNormalized !== normalizedPageUrl;
    });
    ...
    console.log(`[Cannibalization] Found ${competingPages.length} competing pages for "${keyword}"`);

    // Analyze all competing pages
    const competingPagesAnalyzed = [];
    for (const comp of competingPages) {
      console.log(`[Cannibalization]   - Analyzing competing page: ${comp.url}`);
      const pageData = await analyzePage(comp.url, keyword, false);
      if (pageData) {
        competingPagesAnalyzed.push({
          url: comp.url,
          position: comp.position,
    ...
    return {
      hasConflict: true,
      keyword: keyword,
      competingPages: competingPagesAnalyzed
    };

  } catch (error) {
    console.error('[Cannibalization] Error:', error);
    return {
      hasConflict: false,
      competingPages: [],
      error: error.message
...
function analyzeContentGaps(yourPage, competitors) {
  ...
  // 2. Find missing AI optimization elements
  const missingAIOptimization = [];
  
  if (!yourPage.aiOptimization.hasFAQSchema) {
    const competitorsWithFAQ = competitors.filter(c => c.aiOptimization && c.aiOptimization.hasFAQSchema).length;
  ...
  if (!yourPage.aiOptimization.hasBrandBlock) {
    const competitorsWithBrand = competitors.filter(c => c.aiOptimization && c.aiOptimization.hasBrandBlock).length;
  ...
  if (!yourPage.aiOptimization.hasComparisonSnippet) {
    const competitorsWithComparison = competitors.filter(c => c.aiOptimization && c.aiOptimization.hasComparisonSnippet).length;
...
// Get Claude analysis
async function getClaudeAnalysis(yourPage, competitors, keyword, userPosition = null, ubersuggestImage = null, contentGaps = null, cannibalizationData = null) {
  ...
  const prompt = buildAnalysisPrompt(yourPage, competitors, keyword, userPosition, ubersuggestImage, contentGaps, cannibalizationData);
...
function buildAnalysisPrompt(yourPage, competitors, keyword, userPosition = null, hasUbersuggestImage = false, contentGaps = null, cannibalizationData = null) {
  ...
  // Cannibalization section - CRITICAL
  const cannibalizationSection = cannibalizationData && cannibalizationData.hasConflict ? `

⚠️ KEYWORD CANNIBALIZATION DETECTED - THIS IS CRITICAL!

${cannibalizationData.competingPages.length} OF YOUR PAGES ARE COMPETING FOR "${keyword}":

YOUR PAGE (analyzing now):
- URL: ${yourPage.url}
- Position: ${userPosition || 'Not in top 10'}
- Word count: ${yourPage.wordCount}
- Keyword density: ${yourPage.keywordDensity}%

COMPETING PAGES (your own pages):
${cannibalizationData.competingPages.map((page, idx) => `
${idx + 1}. Position ${page.position.toFixed(1)}: ${page.url}
   - Word count: ${page.wordCount}
   - Keyword density: ${page.keywordDensity}%
   - Impressions: ${page.impressions}/mo
   - Clicks: ${page.clicks}/mo`).join('')}

CRITICAL INSTRUCTIONS FOR CANNIBALIZATION:
You MUST provide a clear decision section at the very TOP of your output:

🚨 CANNIBALIZATION DECISION

Winner Page: [choose the URL that should rank]
Reason: [ONE sentence why this is the best page - consider: highest impressions, best content quality, most relevant URL structure]

Action for Losing Pages:
${cannibalizationData.competingPages.map((page, idx) => `
${idx + 1}. ${page.url}
   Decision: [301 REDIRECT to winner] OR [CONSOLIDATE content then 301 redirect]
   
   If CONSOLIDATE:
   - Content to move: [list specific H2 sections or unique content from this page]
   - Where to add it: [specific placement on winner page]
   
   301 Redirect Implementation:
   [Give EXACT .htaccess or Shopify redirect code]
   Example for Shopify:
   Dashboard → Online Store → Navigation → URL Redirects
   Redirect from: ${page.url.replace(/^https?:\/\/[^\/]+/, '')}
   Redirect to: [winner URL path]
`).join('')}

After fixing cannibalization, THEN proceed with normal optimization recommendations.
...
CRITICAL INSTRUCTIONS FOR MISSING ELEMENTS:
