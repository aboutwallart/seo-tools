...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (keywordScreenshot) {
      ...
      console.log(`[Money Page] Screenshot data length: ${keywordScreenshot.length} chars`);
    }
    ...
    if (keywordScreenshot) {
      try {
        ...
        if (keywordAnalysis) {
          console.log(`[Money Page] ✓ Keyword analysis complete: ${keywordAnalysis.recommendation} (${Math.round((Date.now() - startTime) / 1000)}s elapsed)`);
        } else {
          console.log('[Money Page] ⚠️ Keyword analysis returned null');
        }
      } catch (error) {
        console.error('[Money Page] ⚠️ Keyword analysis failed:', error.message);
        // Continue with regular analysis even if keyword analysis fails
      }
    }
...
async function analyzeKeywordScreenshot(base64Image, currentKeyword) {
  ...
  // Detect image type from base64 header if present, otherwise default to png
  ...
  console.log(`[Keyword Analysis] Detected image type: ${mediaType}`);
